import { test, expect, TEST_CARD } from '../../fixtures/test-data';
...
test('registers and places an order', { tag: '@smoke' }, async ({ page, signupData }) => {
  ...
  await test.step('AC-08.2 the logged-out cart is now the account cart', async () => {
    ...
    await expect(cart.rows).toHaveCount(2);
    await expect(cart.row(BLUE_TOP).root).toBeVisible();
  ...
  await test.step('AC-10.3 the review lists the products and totals them', async () => {
    await expect(checkout.reviewRows.filter({ hasText: 'Blue Top' })).toContainText('Rs. 500');
    await expect(checkout.reviewRows.filter({ hasText: 'Men Tshirt' })).toContainText('Rs. 400');

    // Total is the sum of the line totals, not a number the page invented.
    await expect(checkout.totalAmount).toHaveText('Rs. 900');
  });
...
test('AC-10.2 checkout shows the registered address', { tag: '@regression' }, async ({ loggedInPage, account }) => {
  ...
  await products.viewCart();
  await cart.checkout();
  await loggedInPage.waitForURL('**/checkout');
...
test('AC-11.1 downloads the invoice for a placed order', { tag: '@regression' }, async ({ loggedInPage, account }) => {
  ...
  await loggedInPage.waitForURL('**/checkout');
  await checkout.goToPayment();
  await checkout.pay(TEST_CARD);

  const [download] = await Promise.all([
    loggedInPage.waitForEvent('download'),
    checkout.downloadInvoice.click(),
  ]);

  expect(download.suggestedFilename()).toBe('invoice.txt');
  const contents = await download.createReadStream().then(
    (stream) =>
  ...
  expect(contents).toBe(
    `Hi ${account.firstName} ${account.lastName}, Your total purchase amount is 500. Thank you`,
  );
});
